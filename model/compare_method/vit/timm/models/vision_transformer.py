...
class VisionTransformer(nn.Module):
    ...
    A PyTorch impl of : `An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale`
        - https://arxiv.org/abs/2010.11929
    """
    dynamic_img_size: Final[bool]#fimal表示它在初始化后不可修改
    ...
    ) -> None:#在Python中，函数的返回类型可以通过箭头（->）后面的注释来指定。在这种情况下，-> None 表示该函数不返回任何值，也就是说，它的返回值为 None。在Python中，如果函数没有显式使用 return 语句来返回某个值，那么它默认会返回 None。因此，即使没有指定返回类型注释，如果函数没有返回任何值，其返回类型也会被隐式地视为 None。
        """
        Args:
